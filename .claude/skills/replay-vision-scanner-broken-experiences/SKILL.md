...
```jsonc
{
  ...
  "query": {
    ...
    "properties": [
      { "key": "$current_url", "value": "{{COMPLETION_ROUTE}}", "operator": "icontains", "type": "event" }
    ]
  },
  "sampling_rate": 0.5,
...
Before invoking `vision-scanners-create`, replace `{{COMPLETION_ROUTE}}` with the agent-identified completion route.
