...
| `unlink` confirmation                                            | Prompt y/n                                                       | Requires `--yes`; this is the exception where `--yes` is meaningful, but it is not human approval |
| All other mutation confirmations (`config patch` / `put`, `api -X POST/PATCH/DELETE`, `users create`, `enable` / `disable`) | Prompt y/n | **Silently skipped - the mutation executes.** Obtain explicit user confirmation immediately before execution; `--yes`, agent mode, and `--dry-run` are not human approval. |
...
**Rule of thumb:** use `--dry-run` to preview mutations, then obtain explicit user confirmation immediately before executing them. `--yes` is generally unnecessary in agent mode and is never human approval; retain it only where the command documents it as meaningful, such as `unlink`. Use `--json` for structured output where available. Pass `--app` / `--instance` when you intentionally target a real app. Bootstrapping without authentication needs no flag at all — temporary development keys are the default; `--keyless` forces that path over both a signed-in session and an existing linked profile.
...
- **Don't treat `--yes` as approval** - preview mutations with `--dry-run` and obtain explicit user confirmation immediately before execution. In agent mode, `--yes` is generally unnecessary except where documented as meaningful, such as `unlink`.
