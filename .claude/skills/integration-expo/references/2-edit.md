...
Where possible, call PostHog's identify() on the client side after login and signup with a stable user ID. Pass only an explicit allowlist of non-secret properties; never include passwords, one-time codes, tokens, or any other authentication credentials. If there is server-side code, pass the client-side session and distinct ID to the server-side code to preserve client/server correlation. On the server side, make sure events have a matching distinct ID where relevant.
