...
PostHog is configured in `src/config/posthog.ts` using `app.config.js` extras populated from `process.env.POSTHOG_PROJECT_TOKEN` and `process.env.POSTHOG_HOST`:
...
export default function RootLayout() {
  ...
  useEffect(() => {
    if (previousPathname.current !== pathname) {
      const allowedRouteParams = ['from', 'tab', 'taskId']
      const safeRouteParams = Object.fromEntries(
        allowedRouteParams
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const incrementQueue = useRef(Promise.resolve())
  const incrementBurritoConsiderations = async () => {
    incrementQueue.current = incrementQueue.current.then(async () => {
      if (!user) return
      const storedUsername = await storage.getCurrentUser()
      const currentUser = storedUsername ? await storage.getUser(storedUsername) : null
