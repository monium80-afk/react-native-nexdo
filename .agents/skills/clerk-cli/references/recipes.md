...
For every mutating recipe, run a `--dry-run` preview first, review it, then ask the user for explicit confirmation immediately before running the separate real command. Do not treat `--yes`, agent mode, or the preview as human approval; use `--yes` only where a command specifically requires it after confirmation.
...
# Confirm, then run without --dry-run.
