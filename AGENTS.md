...
We are building a modern, intelligent AI task management mobile app** using **Expo**.

The app helps users capture, organize, and execute their workload effortlessly through intelligent features that include:

- Natural multi-modal input (text, voice, images, and documents) to automatically extract and structure actionable tasks
- AI Chat & Smart Inbox for dynamic task creation, conversational schedule management, and instant decision assistance
- Smart AI Prioritization that continuously analyzes deadlines, effort, and available time to suggest the single best next action ("What should I do now?")
- Interactive Task Dashboard with flexible filtering (time available, priority, overdue) and dynamic subtask decomposition
- Smart Rescheduling workflows that actively guide users on handling overdue tasks to prevent backlog overwhelm
- Clean, mobile-first productivity UI engineered for seamless capture, minimal friction, and intuitive interaction
...
- it represents a clear UI concept like TaskCard, PriorityBadge, InboxInput, or PrimaryButton
...
import emptyInbox from "@/assets/images/empty-inbox.png";
import aiSparkle from "@/assets/images/ai-sparkle.png";
...
```tsx
<Image source={images.emptyInbox} />
```
...
Use this for mock tasks, default settings, prompt templates, or seed data.
...
-current task list & subtask states
-AI Inbox & chat conversation history
-user preferences & default app settings (working hours, productivity style)
-dynamic priorities & scheduling recommendations
-active filters & sorting preferences

Use AsyncStorage persistence via Zustand middleware (persist) where local state needs to be retained across app restarts.
